...
int hms_optics(const cmdOptions::OptionParser_hmsOptics& cmdOpts) {
  ...
  for (const auto& runConf : conf.runConfigs) {  // run loop
    ...
    cout<<"minx "<<minx<<" maxx "<<maxx<<" binsx "<<binsx<<endl;
    TH1D zVerHist(
		  TString::Format("zVer"),
    ...
    for (size_t iFoil=0; iFoil<nFoils; ++iFoil) {  // foil loop
      ...
      tmpHist->Draw();
      std::vector<Peak> xSievePeaksFit = fitMultiPeak(tmpHist, 0.07);
      gPad->Update();
	...
	double yComparison = -10.0;
	  ...
	  int binYmin = xySieveHist.GetYaxis()->FindBin(ySievePeak.mean - 3*yPeakSigmaInit);
          ...
          // Want to have at least 100 events for fitting.
    ...
    for (const auto& event : events) {  // SVD filling loop
      ...
      h2_yTar->Fill(zFoil*sinTheta, event.yTar-zFoil*sinTheta);
